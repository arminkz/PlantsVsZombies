...
public class testPlantVsZombie {
	 ...
	 */
	@Test
	public void testPeaAdvance() {
		
		
		GamePanel gamePanelMock = new GamePanel(new JLabel ("hello"));
		int lane = 1;
		int xPos = 1;
...
//		when(gamePanelMock.getLaneZombies().get(lane).get(0)).thenReturn(new Zombie(gamePanelMock,2));
		Pea pea = new Pea(gamePanelMock,xPos,lane);
//		Zombie zomb = new Zombie(gamePanelMock,1);
		pea.advance();

		assertEquals("pea.getPosX() should return \"16 \" ", 15 , pea.getPosX());
	}
